...
x86_frame_cache (struct frame_info *next_frame, void **this_cache, int wordsize)
{
  ...
  *this_cache = cache;
  ...
  /* If we don't know where this function starts, we have to make a guess
     -- and so let's assume that this function did set up a stack frame, as
  ...
  if (get_frame_type (next_frame) != SENTINEL_FRAME)
    {
      ...
      if (cache->frame_base == 0 || prev_frame_pc == 0)
        {
          *this_id = null_frame_id;
          return;
        }
